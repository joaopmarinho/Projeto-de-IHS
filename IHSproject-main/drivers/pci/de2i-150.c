...
static long int my_ioctl(struct file*, unsigned int cmd, unsigned long arg)
{
	...
	case RD_SWITCHES:
		...
		rd_name_idx = IDX_SWITCH;//v
		break;
	case RD_PBUTTONS:
		read_pointer = bar0_mmio + 0x0000; //TODO: update offset
		rd_name_idx = IDX_PBUTTONS;//v
		break;
	case WR_L_DISPLAY:
		write_pointer = bar0_mmio + 0xc000; //TODO: update offset
		wr_name_idx = IDX_DISPLAYL;//v
		break;
	case WR_R_DISPLAY:
		write_pointer = bar0_mmio + 0xc040; //TODO: update offset
		wr_name_idx = IDX_DISPLAYR;//v
		break;
	case WR_RED_LEDS:
		write_pointer = bar0_mmio + 0x0010; //TODO: update offset
		wr_name_idx = IDX_DISPLAYR;//v
		break;
	case WR_GREEN_LEDS:
		write_pointer = bar0_mmio + 0x0020; //TODO: update offset
		wr_name_idx = IDX_DISPLAYR;//v
		break;
	default:
